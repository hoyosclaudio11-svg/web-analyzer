<?php
/**
 * River Plate Info — Club de Fútbol Profesional
 * Setup: páginas, menú, logo, SportsPress + CSS integral
 * Versión: 2026-05-14
 */
if (!defined("ABSPATH")) exit;

/* ================================================================
   SETUP INICIAL — Se ejecuta una sola vez
   ================================================================ */
add_action("init", "wa_river_club_setup", 999);
function wa_river_club_setup() {
    if (get_option("wa_river_club_setup_done")) return;

    // Incluir funciones necesarias de wp-admin
    ...
    $pages = [
        "inicio" => [
            "title" => "Inicio",
            "content" => "<!-- wp:cover {\"url\":\"\",\"id\":0,\"dimRatio\":50,\"overlayColor\":\"black\",\"minHeight\":450,\"align\":\"full\",\"className\":\"river-hero\"} -->
...
",
        ...
        "plantilla" => [
            "title" => "Plantilla",
            "content" => "<!-- wp:heading {\"className\":\"river-section-title\"} -->
<h2>Plantilla Profesional</h2>
<!-- /wp:heading -->
...
<!-- wp:paragraph --><p>Próximamente: camisetas oficiales, merchandising y más productos del Más Grande.</p><!-- /wp:paragraph -->
...
<!-- wp:paragraph --><p>¿Querés anunciar en River Plate Info? ¿Tenés una primicia? Escribinos.</p><!-- /wp:paragraph -->
...
"
    ...
    foreach ($menu_items_order as $i => $slug) {
        if (!isset($page_ids[$slug])) continue;
        // Ver si ya existe en el menu
    ...
    // Asignar menu a ubicacion primaria
    $locs = get_registered_nav_menus();
    $primary_key = "primary";
    if (!isset($locs["primary"])) {
        // Buscar la primera ubicacion disponible
...
/* ================================================================
   CSS INTEGRAL — Club de Fútbol Profesional
   ================================================================ */
...
/* ============================================
   HEADER — Negro con banda roja y trofeos
   ============================================ */
...
.river-hero .wp-block-cover__inner-container {
    position:relative !important;
    z-index:2 !important;
}
.river-hero h1 {
...
.river-hero:after {
    content:"\1F3DF  EL MONUMENTAL" !important;
    position:absolute !important;
...
/* ============================================
   CARDS DE NOTICIAS — Blanco, limpias
   ============================================ */
...
.sp-data-table tbody tr:hover td,.sp-league-table tbody tr:hover td {
    background:#fafafa !important;
}
.sp-data-table tbody tr:nth-child(even) td {
    background:#f9f9f9 !important;
}
/* Destacar River en tabla */
.sp-data-table tbody tr:first-child td,
...
.sp-highlight,.sp-data-table .data-name a {
    color:#111 !important;
    font-weight:700 !important;
}

/* Player List / Plantilla */
.sp-player-list .sp-player {
...
.sp-event .sp-event-name,.sp-event .sp-teams {
    font-weight:700 !important;
    color:#111 !important;
}
.sp-event .sp-event-date,.sp-event .sp-event-time {
    color:#888 !important;
    font-size:12px !important;
}
.sp-event .sp-event-results,.sp-event .sp-event-score {
    font-size:28px !important;
    font-weight:900 !important;
    color:#e63946 !important;
    margin:10px 0 !important;
}
.sp-event .sp-event-venue {
    font-size:12px !important;
    color:#888 !important;
}

/* Fixtures */
.sp-fixtures-list .sp-fixture {
    background:#fff !important;
    border:1px solid #e8e8e8 !important;
...
.sp-fixtures-list .sp-fixture:hover {
    border-color:#e63946 !important;
    background:#fafafa !important;
}
...
/* ============================================
   FOOTER — Negro total con trofeos
   ============================================ */
